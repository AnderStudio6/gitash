...
async function view(interaction, user) {
    ...
    var embedString = `**Discord**\nUsuario: **${currentName}**\nCuenta Creada:**${moment(user.createdAt).format('MM/DD/YY')}**\n\n**Roblox**\n`
    ...
    if (isVerified) {
        ...
        embedString = embedString + `Usuario: **${body.name}**\nUsuario: **${body.displayName}**\nCuenta Creada: **${accountAge} dias**`
    } else {
        embedString = embedString + '[ERR] El usuario no está verificado con una cuenta Roblox.'
    }
